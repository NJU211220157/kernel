...
blk_t *iwalk(dinode_t *file, uint32_t blk_no) {
  ...
  if (blk_no < NDIRECT) {
    // direct address
    // TODO();
    uint32_t i = file->addrs[blk_no];
    if(i == 0){// no such index
      uint32_t blk = balloc();
      file->addrs[blk_no] = blk;
      return bget(blk);
    }
    else{
      return bget(i);
  ...
  if (blk_no < NINDIRECT) {
    ...
    blk_t* bk = bget(i);
    assert(bk != NULL);
    uint32_t j = bk->u32buf[blk_no]; 
    if(j == 0){
      uint32_t blk2 = balloc();
      bk->u32buf[blk_no] = blk2;
      return bget(blk2);
    }
    else{
      return bget(j);
...
void iappend(dinode_t *file, const void *buf, uint32_t size) {
  ...
  // TODO();
  while(size > 0){
    uint32_t idx = file->size / BLK_SIZE;
    uint32_t off = file->size % BLK_SIZE;
    blk_t* blk = iwalk(file, idx);
    assert(blk != NULL);
    if(size < BLK_SIZE - off){
      memcpy((void *)blk + off, buf, size);
      file->size += size;
      return;
    }
    else{
      uint32_t len = BLK_SIZE - off;
      memcpy((void *)blk + off, buf, len);
      file->size += len;
      buf += len;
      size -= len;
    }
...
void add_file(char *path) {
  ...
  iappend(root, &dirent, sizeof dirent);// write the name and num of inode

...
uint32_t load_arg(PD *pgdir, char *const argv[]) {
  ...
  for (argc = 0; argv[argc]; ++argc) {
    ...
    //TODO();
    char* str = argv[argc];
    stack_top -= (strlen(str) + 1);
    strcpy(stack_top, str);
    size_t va = (PAGE_DOWN(USR_MEM - PGSIZE) | ADDR2OFF(stack_top));
    argv_va[argc] = va;
  }
  ...
  // TODO();
  size_t va_arr = (PAGE_DOWN(USR_MEM - PGSIZE) | ADDR2OFF(stack_top));
  stack_top -= sizeof(size_t);
  *(size_t*)stack_top = va_arr;

...
static inode_t *ilookup(inode_t *parent, const char *name, uint32_t *off, int type) {
  ...
  iwrite(parent, empty, &newdir, sizeof dirent);
  if(off != NULL) {
    *off = empty;
  }
  return inode;
...
inode_t *iopen(const char *path, int type) {
  ...
  if(type != TYPE_NONE) assert(inode != NULL);
  iclose(parent);
  return inode;
}
...
static uint32_t iwalk(inode_t *inode, uint32_t no) {
  ...
  if (no < NDIRECT) {
    // direct address
    //TODO();
    uint32_t idx = inode->dinode.addrs[no];
    if(idx == 0){
      uint32_t blk = balloc();
      inode->dinode.addrs[no] = blk;
      iupdate(inode);
      return blk;
    }
    else{
  ...
  if (no < NINDIRECT) {
    // indirect address
    // TODO();
    uint32_t idx = inode->dinode.addrs[12];
    if(idx == 0){
      uint32_t blk = balloc();
      inode->dinode.addrs[12] = blk;
      idx = blk;
      iupdate(inode);
    }
    uint32_t fx;
...
int iread(inode_t *inode, uint32_t off, void *buf, uint32_t len) {
  ...
  uint32_t end_blk = end / BLK_SIZE;
  while(off < end){
    uint32_t blk_idx = off / BLK_SIZE;
    uint32_t blk_off = off % BLK_SIZE;
    uint32_t blk_no  = iwalk(inode, blk_idx);
    if(blk_idx < end_blk){
      bread(buf, BLK_SIZE - blk_off, blk_no, blk_off);
      buf += BLK_SIZE - blk_off;
      off += BLK_SIZE - blk_off;
    }
    else{
      assert(blk_idx == end_blk);
      bread(buf, end - off, blk_no, blk_off);
      off = end;
    }
  }
  return end - start;
...
int iwrite(inode_t *inode, uint32_t off, const void *buf, uint32_t len) {
  ...
  if(isize(inode) < off + len){
    inode->dinode.size = off + len;
    iupdate(inode);
  }
  uint32_t start = off;
  uint32_t size = isize(inode);
  uint32_t end = (off + len) >= size ? size : (off + len);
  uint32_t end_blk = end / BLK_SIZE;
  while(off < end){
    uint32_t blk_idx = off / BLK_SIZE;
    uint32_t blk_off = off % BLK_SIZE;
    uint32_t blk_no  = iwalk(inode, blk_idx);
    if(blk_idx < end_blk){
      bwrite(buf, BLK_SIZE - blk_off, blk_no, blk_off);
      off += (BLK_SIZE - blk_off);
      buf += (BLK_SIZE - blk_off);
    }
    else{
      assert(blk_idx == end_blk);
      bwrite(buf, end - off, blk_no, blk_off);
      off = end;
    }
  }
  return end - start;
...
int iremove(const char *path) {
  ...
  uint32_t off;
  inode_t* fp = ilookup(dir, name, &off, TYPE_NONE);
  if(fp == NULL){
    iclose(dir);
    return -1;
  }
  if(fp->dinode.type == TYPE_DIR){
    if(idirempty(fp) == 0){//not empty
      iclose(dir);
      iclose(fp);
      return -1;
    }
  ...
  iwrite(dir, off, &empty, sizeof empty);
  // iclose(dir);
  // iclose(fp);
  return 0;
}
